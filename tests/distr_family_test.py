...
class TestDistrFamily(unittest.TestCase):
    ...
    def test_mix_DistrFamily(self):
        ...
        self.assertTrue(hasattr(d, 'pdf') and hasattr(d, 'rand'))

    def test_trunc(self):
        P = DistrFamily(['sigma'], lambda p: NormalDistr(0, p['sigma']))
        P_trunc = P.trunc(-3.0, 3.0)
        d = P_trunc.instantiate({'sigma': 2.0})
        self.assertIsInstance(d, TruncDistr)
        self.assertEqual(d.range(), (-3.0, 3.0))

    def test_censor(self):
        P = DistrFamily(['sigma'], lambda p: NormalDistr(0, p['sigma']))
        P_censor = P.censor(-3.0, 3.0)
        d = P_censor.instantiate({'sigma': 2.0})
        self.assertIsInstance(d, CensoredDistr)
        self.assertEqual(d.range(), NormalDistr(0, 2).range())
